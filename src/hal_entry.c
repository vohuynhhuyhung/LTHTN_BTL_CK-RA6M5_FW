...
/* ── Giá trị raw thực tế tương ứng với range scaler ────────
 * Sensor0: IAQ   [0.857  ~ 3.108]
 * Sensor1: TVOC  [0.011  ~ 0.678]
 * Sensor2: eCO2  [399.96 ~ 554.28]
 * Sensor3: EtOH  [0.006  ~ 0.361]
 * ─────────────────────────────────────────────────────────── */
...
////        uart_send_buf((uint8_t *)uart_str, (uint32_t)len);
//    }
	if (!my_prj_init())  /* xử lý lỗi */ return;
    IaqReading_t reading;
    while (1)
    {
        read_iaq_task(&reading);   // blocking ~3s
        ML_handle_task();
        Uart_pkt_update();
